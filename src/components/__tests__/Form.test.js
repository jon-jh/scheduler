...
describe("Form", () => {
  ...
  // Remember the 'const' or variables need to be inside the test function like this, so that each test has it's own isolated scope. Did not work otherwise.
  ...
  it("validates that the interviewer cannot be null", () => {
    ...
    const { getByText } = render(
      <Form interviewers={interviewers} onSave={onSave} name="Some Name" />
    );
  ...
  it("calls onSave function when the name and interviewer is defined", () => {
    ...
    expect(onSave).toHaveBeenCalledWith("Some Name", 1);
  });
});
